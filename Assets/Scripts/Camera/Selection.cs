...
public class Selection : MonoBehaviour
{
    ...
    void TryAddUnit(Unit unit){
        if(unit.isOwnedByPlayer()){
            selectedUnits.Add(unit);
            unit.SetMarkerActive(true);
        }
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButtonUp(0))
        {
                ...
                if(Physics.Raycast(ray,out hit, 50000.0f))
                {
                    if (Input.GetKey(KeyCode.LeftShift)) //inclusive select
                    {
                        if(hit.transform.gameObject.tag=="Unit"){
                            TryAddUnit(hit.transform.gameObject.GetComponent<Unit>());
                        }
            ...
            else //marquee select
            {
                ...
                foreach(Unit unit in MatchManager.instance.playerUnits){
        ...
        if(Input.GetMouseButtonDown(1)){ // Deplacement Unité
            ...
            if(Physics.Raycast(ray,out hit)){
                if(selectedUnits!=null){
                    Transform u = null;
                    if(hit.transform.tag=="Unit" && !hit.transform.GetComponent<Unit>().isOwnedByPlayer()){
                        u = Utilities.FindParent(hit.transform,"Unit");
                    }
                    Vector3 pos = new Vector3(hit.point.x,hit.point.y,hit.point.z);
                    foreach(Unit unit in selectedUnits){
                        if(unit.UnitInRange(u)){
                            unit.SetCurrentTarget(u);
                        }else{
                            unit.GoTo(pos);
                        }
    ...
    private void OnTriggerEnter(Collider other)
    {
        if(other.gameObject.tag=="Unit"){
            TryAddUnit(other.gameObject.GetComponent<Unit>());
        }
    }
